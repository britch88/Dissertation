...
adv <-felm(`Poor mental health days raw value` ~  young_adult_rate + lag.ya.rate +
...
# Disadvantaged ----
dis <-felm(`Poor mental health days raw value` ~  young_adult_rate + lag.ya.rate +
             `% 65 and older raw value` +  `% below 18 years of age raw value` + `% Females raw value` + 
             `% American Indian and Alaskan Native raw value` + `% Asian raw value` + 
             `% Non-Hispanic African American raw value` +   `% Hispanic raw value` +   
             `% Native Hawaiian/Other Pacific Islander raw value` + 
...
saveRDS(all.mods, here::here("rda","advantage_subgroup_crfe_mental.rds"))


write.csv(all.mods, here::here("Output","advantage_subgroup_crfe_mental.csv"))
