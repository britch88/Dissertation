...
#Expected value is 3,141 records, one for each county-level fips code. However, there are some duplicates, resulting in N=3,197. Below we find that these are all instances where the "name" column was missing for one record but non-missing in the other. We select record with non-missing value of name. 
...
#If we need to keep 'id's that are NA without taking the distinct of those
...
county.prob.plus.info <- merge(best_mod_names, dat.temp2, by = "fips_clean")
saveRDS(county.prob.plus.info,"rda/county_prob_plus_info2.rds")
                                                          
                                                          #'c_young_adult_tot','c_other_adult_tot','c_all_adult_tot','n_other_adults','n_young_adults','n_all_adults','Name','young_adult_rate','other_adult_rate','all_adult_rate','young_to_other_ratio','country_division'
...
saveRDS("long_trajectory_results.rds")
saveRDS("weighted_trajectory_results.rds")
