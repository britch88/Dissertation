...
lm1 <- lm(`Poor or fair health raw value` ~  young_adult_rate + lag.ya.rate + fips_clean,
             data = health.dat2)
...
lm2 <- lm(`Poor or fair health raw value` ~  young_adult_rate + lag.ya.rate +
...
lm3 <- lm(`Poor or fair health raw value` ~  young_adult_rate + lag.ya.rate + 
               `% 65 and older raw value` +  `% American Indian and Alaskan Native raw value` +
               `% below 18 years of age raw value` + `% Females raw value` + 
               `% Native Hawaiian/Other Pacific Islander raw value` +  `% Non-Hispanic African American raw value` +
...
saveRDS(all.mods, here::here("rda","fe_poorfair"))


write.csv(all.mods, here::here("Output","fe_poorfair.csv"))
