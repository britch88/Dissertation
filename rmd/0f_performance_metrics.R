...
conf_metrics_thres_learner <- function(predProbs, outcomeName, thresSeq, equityVar = NULL)
{
  ...
  if(is.null(equityVar))
  {
    ...
    confData <- predProbs %>%
      dplyr::select(learnerName, pred_1, !!outcomeName,
                    ML, predSet, tuningSet)
    
    for(learnerName in learnerNames)
    {
      confDataLearner <- confData %>%
        filter(learnerName == learnerName)
      
      confOutLearner <- conf_metrics_thres(
        predictions = confDataLearner$pred_1,
        outcomes = confDataLearner[[outcomeName]],
        thresSeq = thresSeq
      )
      confOutLearner$learnerName <- learnerName
  ...
  } else
  {
    ...
    confData <- predProbs %>%
      dplyr::select(learnerName, pred_1, !!outcomeName,
                    ML, predSet, tuningSet, all_of(equityVar))
    ...
    for(learnerName in learnerNames)
    {
      ...
      for(equityVarCat in equityVarCats)
      {
        confDataLearnerEquity <- confData %>%
          filter(learnerName == learnerName) %>%
          filter(.data[[equityVar]] == equityVarCat)
        
        confOutLearner <- conf_metrics_thres(
          predictions = confDataLearnerEquity$pred_1,
