...
health.dat2 <- filter(health.dat, is.na(poor.fair)==0)


# Model 1: Random effects, lag rate and county random effects
m1 <- plm(poor.fair ~ z.lag.ya.rate, data=health.dat2, index=c("fips_clean", "year"), model="random")  #random model
summary(m1)
...
m2 <- plm(poor.fair ~ z.lag.ya.rate + z.homicides + z.black + z.black*z.lag.ya.rate, data=health.dat2, index=c("fips_clean", "year"), model="random")  #random model
summary(m2)
...
m3 <- plm(poor.fair ~ z.lag.ya.rate + z.homicides + z.black + z.black*z.lag.ya.rate + disadvantage.scale, data=health.dat2, index=c("fips_clean", "year"), model="random")  #random model
summary(m3)
...
m4 <- plm(poor.fair ~ z.lag.ya.rate + z.homicides + z.black + z.black*z.lag.ya.rate + disadvantage.score, data=health.dat2, index=c("fips_clean", "year"), model="random")  #random model
summary(m4)
...
m5 <- plm(poor.fair ~ z.lag.ya.rate + z.homicides + z.black + z.black*z.lag.ya.rate + 
...
saveRDS(all.mods, here::here("rda","poor_fair_RE_202309.rds"))


write.csv(all.mods, here::here("Output","poor_fair_RE_202309.csv"))
