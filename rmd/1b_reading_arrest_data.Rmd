...
```{r}
#Read in charge data and create variables
dat.prop1 <- readRDS(here::here("raw/Main Arrests Data/ucr_arrests_yearly_index_crimes_age_1974_2019.rds"))
...
# Create variable capturing young adult total index charges
    dat.prop3 <- dat.prop2 %>% 
      mutate(young_adult_tot = select(dat.prop2, 
                                        contains("_18"),
                                  contains("_19"),
                                  contains("_20"),
                                  contains("_21"),
...
# Create variable capturing other adult total index charges
    dat.prop4 <- dat.prop3 %>% 
      mutate(other_adult_tot = select(dat.prop3, 
                                      contains("_25"),
                                  contains("_30"),
                                  contains("_35"),
                                  contains("_40"),
...
dat.check2 = filter(dat.prop4, year==2018, fips_state_county_code == "02020", agency_name == "anchorage") %>% 
  select( year, fips_state_county_code, other_adult_tot,
                                        contains("_25"),
                                  contains("_30"),
                                  contains("_35"),
                                  contains("_40"),
...
dat.prop5 <- dat.prop4 %>% 
  select(-contains("_18"),
         ...
         -contains("_tot_"),
         -contains("num_")) %>% 
  mutate(all_adult_tot = young_adult_tot + other_adult_tot)
...
save(dat7, file = file.path(rdadir, "population_wide.RData"))
```
