...
analysis.dat3<- analysis.dat2 %>% 
  ...
  arrange(fips_clean)  %>% select(sort(names(analysis.dat3)))


#Turn data into matrix with ID variable omitted
analysis.mat1 <- as.matrix(analysis.dat3[,2:47]) #turn into matrix


summary(analysis.dat3) #check contents
...
out2<-crimCV(test.dat2,2,dpolyp=2,init=5, rcv=FALSE)
