...
hist(disadvantage$`Poor or fair health raw value`, breaks=100)
...
poor.fair.state.year <- lmer(`Poor or fair health raw value` ~ young_adult_rate + lag.ya.rate  +  (1 | fips_clean), 
                             data = disadvantage,
                             REML = FALSE)
...
poor.fair.arrest.demo <- lmer(`Poor or fair health raw value` ~ young_adult_rate + lag.ya.rate +
...
poor.fair.structural <- lmer(`Poor or fair health raw value` ~  young_adult_rate + lag.ya.rate + 
                          `% 65 and older raw value` +  `% American Indian and Alaskan Native raw value` +
                          `% below 18 years of age raw value` + `% Females raw value` + 
                          `% Native Hawaiian/Other Pacific Islander raw value` +  `% Non-Hispanic African American raw value` +
                          `% Hispanic raw value` +  `% Asian raw value` + `% not proficient in English raw value`+ `% Rural raw value` +
...
saveRDS(all.mods.poor.fair, here::here("rda","disadvantaged_lme_poorfair.rds"))


write.csv(all.mods.poor.fair, here::here("Output","disadvantaged_lme_poorfair.csv"))
