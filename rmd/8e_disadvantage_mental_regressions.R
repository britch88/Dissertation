...
hist(disadvantage$`Poor mental health days raw value`, breaks=100)


# Running Models: Poor mental health days    ----


#### Arrests with state and year FE
poor.fair.state.year <- lm(`Poor mental health days raw value` ~ young_adult_rate + lag.ya.rate  + as.factor(year) + state ,
                           data = disadvantage)
...
poor.fair.arrest.demo <- lm(`Poor mental health days raw value` ~ as.factor(year) + state +  young_adult_rate + lag.ya.rate +
...
poor.fair.structural <- lm(`Poor mental health days raw value` ~ as.factor(year) + state + 
                          `% 65 and older raw value` +  `% American Indian and Alaskan Native raw value` +
                          `% below 18 years of age raw value` + `% Females raw value` + 
                          `% Native Hawaiian/Other Pacific Islander raw value` +  `% Non-Hispanic African American raw value` +
                          `% Hispanic raw value` +  `% Asian raw value` + `% not proficient in English raw value`+ `% Rural raw value` + 
...
saveRDS(all.mods.poor.fair, here::here("rda","disadvantaged_health_results_mental.rds"))


write.csv(all.mods.poor.fair, here::here("Output","disadvantaged_health_results_mental.csv"))
