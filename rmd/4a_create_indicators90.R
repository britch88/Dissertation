...
combo.dat7 <- combo.dat6 %>% 
  mutate(bankrate = nbanks/pop2017,
         ...
         poverty_2017.gtp90 = poverty_2017 < quantile(poverty_2017,.10, na.rm = TRUE),
         
         unemployment_rate_2017.gtp90 = unemployment_rate_2017 < quantile(unemployment_rate_2017, .10, na.rm = TRUE),

         median_household_income_2017.gtp90 = median_household_income_2017 > quantile(median_household_income_2017, .90, na.rm = TRUE),

         some_college_2017.gtp90 = some_college_2017 > quantile(some_college_2017, .90, na.rm = TRUE),

         hs_grad_2017.gtp90 = hs_grad_2017 > quantile(hs_grad_2017, .90, na.rm = TRUE),
         ...
         disconnected.2017.gtp90 = ifelse(is.na(`% Disconnected Youth`) == 0 & `% Disconnected Youth` < quantile(`% Disconnected Youth`, .10, na.rm = TRUE),1,0),
        ...
        violent.2017.gtp90 = ifelse(is.na(`Violent Crime Rate`) == 0 & `Violent Crime Rate` < quantile(`Violent Crime Rate`, .10, na.rm = TRUE),1,0),

        segregation.2017.na = as.numeric(is.na(segregation_white_nonwhite)),
        segregation.2017.gtp90 = ifelse(is.na(segregation_white_nonwhite) == 0 & segregation_white_nonwhite < quantile(segregation_white_nonwhite, .10, na.rm = TRUE),1,0),

      pcp_2017.gtp90 = `PCP Rate` > quantile(`PCP Rate`, .90, na.rm = TRUE),

      lbw_2017.gtp90 = `% LBW` < quantile(`% LBW`, .10, na.rm = TRUE),

      
      income_ratio_2017.gtp90 = `Income Ratio` < quantile(`Income Ratio`, .10, na.rm = TRUE),

      
      uninsured_2017.gtp90 = `% Uninsured` < quantile(`% Uninsured`, .10, na.rm = TRUE),

      
      under18_2017.gtp90 = `% < 18` > quantile(`% < 18`, .90, na.rm = TRUE),

      
      over64_2017.gtp90 = `% 65 and over` > quantile(`% 65 and over`, .90, na.rm = TRUE),
      
      black_2017.gtp90 = `% African American` > quantile(`% African American`, .90, na.rm = TRUE),
      
      native_2017.gtp90 = `% American Indian/Alaskan Native` > quantile(`% American Indian/Alaskan Native`, .90, na.rm = TRUE),
      
      pacific_islander_2017.gtp90 = `% Native Hawaiian/Other Pacific Islander` > quantile(`% Native Hawaiian/Other Pacific Islander`, .90, na.rm = TRUE),
      
      hispanic_2017.gtp90 = `% Hispanic` > quantile(`% Hispanic`, .90, na.rm = TRUE),
      
      white_2017.gtp90 = `% Non-Hispanic White` > quantile(`% Non-Hispanic White`, .90, na.rm = TRUE),

      lim_eng_2017.gtp90 = `% Not Proficient in English` > quantile(`% Not Proficient in English`, .90, na.rm = TRUE),

      female_2017.gtp90 = `% Female` > quantile(`% Female`, .90, na.rm = TRUE),
      
      rural_2017.gtp90 = `% Rural` > quantile(`% Rural`, .90, na.rm = TRUE),
      
      prek_2017.gtp90 = prekpct > quantile(prekpct, .90, na.rm = TRUE),

      
      rentburden_2017.gtp90 = rentburden_pct < quantile(rentburden_pct, .10, na.rm = TRUE),

      deaths_2017.lt10 = is.na(deaths_age_adj_rate2017),
      deaths_2017.gtp90 = ifelse(is.na(deaths_age_adj_rate2017)==0 & deaths_age_adj_rate2017 < quantile(deaths_age_adj_rate2017, .10, na.rm = TRUE),1,0),

      incarcerated_2017.gtp90 = incarcerated_rate < quantile(incarcerated_rate, .10, na.rm = TRUE),

      voting_2017.gtp90 = voting_pct > quantile(voting_pct, .90, na.rm = TRUE),

      volunteer_2017.gtp90 = volunteer_pct > quantile(volunteer_pct, .90, na.rm = TRUE)) %>% 
...
saveRDS(combo.dat7, here::here("rda","analysis_vi_2017_90th.rds"))
