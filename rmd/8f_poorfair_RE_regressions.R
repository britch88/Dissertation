...
lmm1 <- lmer(`Poor or fair health raw value` ~  young_adult_rate + lag.ya.rate + 
               (1 | fips_clean), 
             data = health.dat2,
             REML = FALSE)
...
lmm2 <- lmer(`Poor or fair health raw value` ~  young_adult_rate + lag.ya.rate +
...
lmm3 <- lmer(`Poor or fair health raw value` ~  young_adult_rate + lag.ya.rate + 
              `% 65 and older raw value` +  `% American Indian and Alaskan Native raw value` +
              `% below 18 years of age raw value` + `% Females raw value` + 
              `% Native Hawaiian/Other Pacific Islander raw value` +  `% Non-Hispanic African American raw value` +
              `% Hispanic raw value` +  `% Asian raw value` + `% not proficient in English raw value`+ `% Rural raw value` + 
...
saveRDS(all.mods.poor.fair, here::here("rda","lme_poor_fair"))


write.csv(all.mods.poor.fair, here::here("Output","lme_poor_fair.csv"))
