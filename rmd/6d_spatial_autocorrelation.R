...
globalMoran <- moran.test(county3v2$rate2019, listw2, na.action =  na.exclude, zero.policy=TRUE)
globalMoran

# Creating Table of Local Moran's 
templmoran <- localmoran(county3v2$rate2019, listw2) #, p.adjust.method="none", adjust.x=TRUE, zero.policy = TRUE)


lmoran<- cbind(county3v2@data, localmoran(county3v2$rate2019, listw2, adjust.x=TRUE, zero.policy = TRUE)) #,  p.adjust.method="none" ))
lmoran

# centers the local Moran's around the mean
lmoran$Ii <- lmoran$Ii - mean(lmoran$Ii, na.rm = TRUE) 
lmoran$lag.rate2019 <-  lag.listw(listw2,lmoran$rate2019, NAOK = TRUE)
...
ggplot(lagdat2, aes(x=rate2019.y, y=lag.rate2019)) + 
  ...
  xlab("Arrest rate in 2019") +
  ylab("Spatial lag of arrest rate in 2019")
