...
## Retrieve group function terms
  ...
  # retrieve model function terms with intercept and * for p < .05
    ...
    long.test.dat$cluster = as.factor(long.test.dat$cluster)
      ...
      #save results
    saveRDS(long.test.dat,"rda/Intermediate/long.test.dat4group.rds")
...
best.mod1 <- crimCV(test.dat2,4,rcv=TRUE, dpolyp =2, dpolyl =3)
plot(best.mod1)
...
```{r}
longD4group <- long_traj(model=best.mod1,data=test.dat2)
x <- weighted_means(model=best.mod1,long_data=longD4group)
...
p <- ggplot(data=longD4group, aes(x=x,y=y,group=Ord)) + geom_line(alpha = 0.1) + facet_wrap(~GMax)
p 
...
```{r}
nonw_means <- aggregate(longD4group$y,by=list(Group=longD4group$GMax,x=longD4group$x),FUN="mean")
...
p4 <- ggplot() + geom_line(data=pred, aes(x=x,y=pred_mean), col='red') + 
                 geom_line(data=longD4group, aes(x=x,y=y,group=Ord), alpha = 0.1) + facet_wrap(~GMax)
p4
