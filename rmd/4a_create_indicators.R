...
combo.dat7 <- combo.dat6 %>% 
  mutate(bankrate = nbanks/pop2017,
         ...
         bankrate2017.ltp25 = ifelse(as.numeric(is.na(bankrate)) == 1,0, ifelse(
         bankrate < quantile(bankrate, .25, na.rm = TRUE),1,0)),
         bankrate2017.gtp75 = ifelse(is.na(bankrate) == 0, bankrate > quantile(bankrate, .75, na.rm = TRUE),0),
         bankrate2017.p25top75 = ifelse(is.na(bankrate) == 0, bankrate <= quantile(bankrate, .75, na.rm = TRUE) 
         ...
         disconnected.2017.ltp25 = ifelse(as.numeric(is.na(`% Disconnected Youth`)) == 1,0, ifelse(
           `% Disconnected Youth` < quantile(`% Disconnected Youth`, .25, na.rm = TRUE),1,0)),
         disconnected.2017.gtp75 = ifelse(is.na(`% Disconnected Youth`) == 0, `% Disconnected Youth` > quantile(`% Disconnected Youth`, .75, na.rm = TRUE),0),
         disconnected.2017.p25top75 = ifelse(is.na(`% Disconnected Youth`) == 0, `% Disconnected Youth` <= quantile(`% Disconnected Youth`, .75, na.rm = TRUE) 
                                    & `% Disconnected Youth` >= quantile(`% Disconnected Youth`,.25, na.rm = TRUE),0),

         
        food.2017.na = as.numeric(is.na(food)),
        food.2017.ltp25 = ifelse(as.numeric(is.na(food)) == 1,0, ifelse(food < quantile(food, .25, na.rm = TRUE),1,0)),
        food.2017.gtp75 = ifelse(is.na(food) == 0, food > quantile(food, .75, na.rm = TRUE),0),
        food.2017.p25top75 = ifelse(is.na(food) == 0, food <= quantile(food, .75, na.rm = TRUE) 
                                            & food >= quantile(food,.25, na.rm = TRUE),0),

        
        violent.2017.na = as.numeric(is.na(`Violent Crime Rate`)),
        violent.2017.ltp25 = ifelse(as.numeric(is.na(`Violent Crime Rate`)) == 1,0, ifelse(
          `Violent Crime Rate` < quantile(`Violent Crime Rate`, .25, na.rm = TRUE),1,0)),
        violent.2017.gtp75 = ifelse(is.na(`Violent Crime Rate`) == 0, `Violent Crime Rate` > quantile(`Violent Crime Rate`, .75, na.rm = TRUE),0),
        violent.2017.p25top75 = ifelse(is.na(`Violent Crime Rate`) == 0, `Violent Crime Rate` <= quantile(`Violent Crime Rate`, .75, na.rm = TRUE) 
                                            & `Violent Crime Rate` >= quantile(`Violent Crime Rate`,.25, na.rm = TRUE),0),
    
        
        segregation.2017.na = as.numeric(is.na(segregation_white_nonwhite)),
        segregation.2017.ltp25 = ifelse(as.numeric(is.na(segregation_white_nonwhite)) == 1,0, ifelse(
          segregation_white_nonwhite < quantile(segregation_white_nonwhite, .25, na.rm = TRUE),1,0)),
        segregation.2017.gtp75 = ifelse(is.na(segregation_white_nonwhite) == 0, segregation_white_nonwhite > quantile(segregation_white_nonwhite, .75, na.rm = TRUE),0),
        segregation.2017.p25top75 = ifelse(is.na(segregation_white_nonwhite) == 0, segregation_white_nonwhite <= quantile(segregation_white_nonwhite, .75, na.rm = TRUE) 
                                       & segregation_white_nonwhite >= quantile(segregation_white_nonwhite,.25, na.rm = TRUE),0),
    
    
      pcp_2017.ltp25 = `PCP Rate` < quantile(`PCP Rate`, .25, na.rm = TRUE),
      pcp_2017.gtp75 = `PCP Rate` > quantile(`PCP Rate`, .75, na.rm = TRUE),
      pcp_2017.p25top75 = `PCP Rate` <= quantile(`PCP Rate`, .75, na.rm = TRUE) 
      & `PCP Rate` >= quantile(`PCP Rate`,.25, na.rm = TRUE),
      
      
      lbw_2017.ltp25 = `% LBW` < quantile(`% LBW`, .25, na.rm = TRUE),
      lbw_2017.gtp75 = `% LBW` > quantile(`% LBW`, .75, na.rm = TRUE),
      lbw_2017.p25top75 = `% LBW` <= quantile(`% LBW`, .75, na.rm = TRUE) 
      & `% LBW` >= quantile(`% LBW`,.25, na.rm = TRUE),
      
      
      income_ratio_2017.ltp25 = `Income Ratio` < quantile(`Income Ratio`, .25, na.rm = TRUE),
      income_ratio_2017.gtp75 = `Income Ratio` > quantile(`Income Ratio`, .75, na.rm = TRUE),
      income_ratio_2017.p25top75 = `Income Ratio` <= quantile(`Income Ratio`, .75, na.rm = TRUE) 
      & `Income Ratio` >= quantile(`Income Ratio`,.25, na.rm = TRUE),
      
      
      uninsured_2017.ltp25 = `% Uninsured` < quantile(`% Uninsured`, .25, na.rm = TRUE),
      uninsured_2017.gtp75 = `% Uninsured` > quantile(`% Uninsured`, .75, na.rm = TRUE),
      uninsured_2017.p25top75 = `% Uninsured` <= quantile(`% Uninsured`, .75, na.rm = TRUE) 
      & `% Uninsured` >= quantile(`% Uninsured`,.25, na.rm = TRUE),
      ...
      prek_2017.ltp25 = prekpct < quantile(prekpct, .25, na.rm = TRUE),
      prek_2017.gtp75 = prekpct > quantile(prekpct, .75, na.rm = TRUE),
      prek_2017.p25top75 = prekpct <= quantile(prekpct, .75, na.rm = TRUE) 
      & prekpct >= quantile(prekpct,.25, na.rm = TRUE),
      
      
      rentburden_2017.ltp25 = rentburden_pct < quantile(rentburden_pct, .25, na.rm = TRUE),
      rentburden_2017.gtp75 = rentburden_pct > quantile(rentburden_pct, .75, na.rm = TRUE),
      rentburden_2017.p25top75 = rentburden_pct <= quantile(rentburden_pct, .75, na.rm = TRUE) 
      & rentburden_pct >= quantile(rentburden_pct,.25, na.rm = TRUE),
      
      deaths_2017.lt10 = is.na(deaths_age_adj_rate2017),
      deaths_2017.ltp25 = ifelse(is.na(deaths_age_adj_rate2017), 0, deaths_age_adj_rate2017 < quantile(deaths_age_adj_rate2017, .25, na.rm = TRUE)),
      deaths_2017.gtp75 = ifelse(is.na(deaths_age_adj_rate2017)==0,deaths_age_adj_rate2017 > quantile(deaths_age_adj_rate2017, .75, na.rm = TRUE),0),
      deaths_2017.p25top75 = ifelse(is.na(deaths_age_adj_rate2017)==0,
                                    deaths_age_adj_rate2017 <= quantile(deaths_age_adj_rate2017, .75, na.rm = TRUE) & deaths_age_adj_rate2017 >= quantile(deaths_age_adj_rate2017,.25, na.rm = TRUE),
                                    0),
      
  
      incarcerated_2017.ltp25 = incarcerated_rate < quantile(incarcerated_rate, .25, na.rm = TRUE),
      incarcerated_2017.gtp75 = incarcerated_rate > quantile(incarcerated_rate, .75, na.rm = TRUE),
      incarcerated_2017.p25top75 = incarcerated_rate <= quantile(incarcerated_rate, .75, na.rm = TRUE) 
      & incarcerated_rate >= quantile(incarcerated_rate,.25, na.rm = TRUE),
      
      
      voting_2017.ltp25 = voting_pct < quantile(voting_pct, .25, na.rm = TRUE),
      voting_2017.gtp75 = voting_pct > quantile(voting_pct, .75, na.rm = TRUE),
      voting_2017.p25top75 = voting_pct <= quantile(voting_pct, .75, na.rm = TRUE) 
      & voting_pct >= quantile(voting_pct,.25, na.rm = TRUE),
      
      
      volunteer_2017.ltp25 = volunteer_pct < quantile(volunteer_pct, .25, na.rm = TRUE),
      volunteer_2017.gtp75 = volunteer_pct > quantile(volunteer_pct, .75, na.rm = TRUE),
      volunteer_2017.p25top75 = volunteer_pct <= quantile(volunteer_pct, .75, na.rm = TRUE) 
      & volunteer_pct >= quantile(volunteer_pct,.25, na.rm = TRUE),) %>% 
  
  fastDummies::dummy_cols(select_columns = c("region", "country_division", "state"))
...
saveRDS(combo.dat7, here::here("rda","analysis_vi_2017.rds"))
