...
twfe1 <- felm(`Poor physical health days raw value` ~  young_adult_rate + lag.ya.rate | fips_clean  | 0 | fips_clean, health.dat2)
summary(twfe1)
...
twfe2 <- felm(`Poor physical health days raw value` ~  young_adult_rate + lag.ya.rate +
...
#### Structural factors
twfe3 <- felm(`Poor physical health days raw value` ~  young_adult_rate + lag.ya.rate +
                `% 65 and older raw value` +  `% American Indian and Alaskan Native raw value` +
                `% below 18 years of age raw value` + `% Females raw value` + 
                `% Native Hawaiian/Other Pacific Islander raw value` +  `% Non-Hispanic African American raw value` +
                `% Hispanic raw value` +  `% Asian raw value` + `% not proficient in English raw value`+ `% Rural raw value` +
...
saveRDS(all.mods, here::here("rda","twfe_physical"))


write.csv(all.mods, here::here("Output","twfe_physical.csv"))
