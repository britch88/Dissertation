...
# load the data set
datvi <-readRDS(here::here("rda","logit_analysis_file.rds"))
...
lows <-rep(c(0),each=83)
highs <-rep(c(1),each=83)
...
vi_est_grp1_summary <- vi_est_grp1 %>% 
...
head(vi_est_grp1_summary)

saveRDS(vi_est_grp1_summary, here::here("rda/vi results","vi_grp1_no_state.rds"))
...
outcome <- unique(vi_est_grp1_summary$outcome)
predictorLabels <- paste0(
  vi_est_grp1_summary$predictor, "\n",
  vi_est_grp1_summary$lowValue, " to ",
  vi_est_grp1_summary$highValue
)
# Plot variable importance 
ggplot2::ggplot(vi_est_grp1_summary,
