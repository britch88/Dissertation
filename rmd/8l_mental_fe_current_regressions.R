...
twfe1 <- felm(`Poor mental health days raw value` ~  young_adult_rate  | fips_clean  | 0 | fips_clean, health.dat2)
summary(twfe1)
...
twfe2 <- felm(`Poor mental health days raw value` ~  young_adult_rate + 
...
#### Structural factors
twfe3 <- felm(`Poor mental health days raw value` ~  young_adult_rate + 
                `% 65 and older raw value` +  `% American Indian and Alaskan Native raw value` +
                `% below 18 years of age raw value` + `% Females raw value` + 
                `% Native Hawaiian/Other Pacific Islander raw value` +  `% Non-Hispanic African American raw value` +
                `% Hispanic raw value` +  `% Asian raw value` + `% not proficient in English raw value`+ `% Rural raw value` +
...
saveRDS(all.mods.poor.fair, here::here("rda","fe_mental_current"))


write.csv(all.mods.poor.fair, here::here("Output","fe_mental_current.csv"))
